...
5.5. **코드 경계 조건 탐색** — 요약을 쓰기 전에 메인이 코드를 한 번 더 본다
...
기술설계 묶음에서 사용자에게 묻는 것은 **사용자가 판단할 수 있는 것**뿐이다 — 사용자가 보게 될 동작 차이, 비용·속도·범위 같은 맞바꿈. 필터를 어느 파일에서 거를지, 출력 모양, 내부 함수 구성 같은 순수 구현 결정은 묻지 않는다. 메인이 추천안으로 정하고 요약의 "구현 방식 결정" 에 적는다. 이 결정들은 10 단계 승인 메시지에 "메인이 정한 기술 결정" 목록으로 한 번에 보여준다.
...
작성 에이전트는 요약만 보고 쓴다. 요약에 없는 코드 사실은 두 문서 어디에도 들어가지 않는다. 그래서 요약을 쓰기 전에 메인이 영향 범위의 코드를 한 번 더 읽고 아래를 확인한다.
...
| 개수·존재 | 요약에 적을 개수 · 파일 · 호출처를 실제로 세어 확인했는가 |

찾은 것은 요약의 "위험" 에 적고, 대응이 기술 판단이면 메인이 추천값으로 정해 "구현 방식 결정" 에 넣는다. `## 미정` 에는 사용자만 정할 수 있는 항목만 남긴다.
...
## 사용자가 보는 동작 결정
| 결정 지점 | 고른 안 | 버린 안 | 무엇이 달라지는가 | 무엇을 포기하는가 | 되돌리는 비용 | 깨지는 조건 |
|---|---|---|---|---|---|---|

## 구현 방식 결정
| 결정 지점 | 고른 안 | 버린 안 | 무엇이 달라지는가 | 무엇을 포기하는가 | 되돌리는 비용 | 깨지는 조건 | 누가 정함 (사용자 / 메인 추천) |
|---|---|---|---|---|---|---|---|
...
두 결정 표는 섞지 않는다. 요구사항 작성자는 "사용자가 보는 동작 결정" 만 문서에 옮기고, "구현 방식 결정" 과 "기술설계" 절은 옮기지 않는다. 기술설계 작성자는 두 표를 모두 옮긴다. 요약의 사실 (개수 · 파일 · 호출처) 은 5.5 에서 실제로 확인한 값만 적는다.
...
두 문서 RAW 전체와 검증 보고서를 한 메시지에 싣는다. 같은 메시지에 "메인이 정한 기술 결정" 목록 (요약의 구현 방식 결정 중 메인 추천으로 정한 것), 9 단계에서 이미 고친 사실 결함 목록, 반영하지 않은 판단 사안 권장 목록을 함께 싣는다. 문서별로 끊어 승인받지 않는다.
...
| 요약에 결정 표 하나만 두고 두 작성자에게 줌 | 사용자가 보는 동작 결정과 구현 방식 결정을 나눈다. 요구사항서에 기술 결정이 새어 들어간다 |
| 코드를 다시 안 보고 요약 작성 | 5.5 코드 경계 조건 탐색 후 확인한 사실만 적는다. 요약의 오류는 두 문서로 복제된다 |
