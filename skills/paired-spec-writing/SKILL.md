...
4. **통합 질문 대화** — 한 번에 하나, 두 문서의 빈칸이 다 찰 때까지. 대안을 내기 전에 그 전제를 저장소 실데이터로 확인
5. **제외 항목 되돌려주기** — 모아서 한 번 보여주고 추가만 받기
5.5. **남은 코드 경계 조건 탐색** — 대화 중 확인하지 못한 나머지를 문서 쓰기 전에 마저 본다
6. **요구사항서 작성** — 요구 번호를 여기서 매긴다. 누가 정했든 사용자가 보게 될 동작은 모두 요구 항목으로
7. **기술설계서 작성** — 요구사항서의 번호와 결정을 그대로 이어 받는다. 빠진 사용자 동작을 발견하면 요구사항서에 먼저 더한다
8. **두 문서 검증을 한 번에** — 검증자 셋을 한 메시지로 띄우고 메인 검증을 함께 돌린다
9. **사실 결함 반영** — 승인 전에 고치고, 고친 자리만 한 번 다시 대조한다
10. **두 문서 한 번에 승인받기** — 전문 대신 경로 + 핵심 요약 (80줄 안팎). 수정 요청이면 두 문서를 함께 고치고 재검증
...
digraph paired_spec_flow {
    ...
    "통합 질문 (한 번에 하나)\n(대안 전 실데이터 확인)" -> "두 문서 빈칸 다 찼나?";
    "두 문서 빈칸 다 찼나?" -> "통합 질문 (한 번에 하나)\n(대안 전 실데이터 확인)" [label="아직"];
    "두 문서 빈칸 다 찼나?" -> "제외 항목 되돌려주기" [label="다 참 / 사용자가 그만"];
    "제외 항목 되돌려주기" -> "남은 코드 경계 조건 탐색";
    "남은 코드 경계 조건 탐색" -> "요구사항서 작성\n(요구 번호 부여)";
    "요구사항서 작성\n(요구 번호 부여)" -> "기술설계서 작성";
    "기술설계서 작성" -> "요구사항서 작성\n(요구 번호 부여)" [label="빠진 사용자 동작 발견"];
    "기술설계서 작성" -> "두 문서 검증을 한 번에\n(검증자 셋 한 메시지 + 메인 A·C)";
    "두 문서 검증을 한 번에\n(검증자 셋 한 메시지 + 메인 A·C)" -> "사실 결함 반영\n(고친 자리만 한 번 재대조)";
    "사실 결함 반영\n(고친 자리만 한 번 재대조)" -> "결합 승인\n(경로 + 요약)";
    "결합 승인\n(경로 + 요약)" -> "두 문서 함께 수정" [label="수정"];
    "두 문서 함께 수정" -> "두 문서 검증을 한 번에\n(검증자 셋 한 메시지 + 메인 A·C)";
    "결합 승인\n(경로 + 요약)" -> "변경이력 두 건" [label="승인"];
    "변경이력 두 건" -> "산출물 깊이 선택";
...
**대안을 내기 전에 전제를 저장소 실데이터로 확인한다.** 방향을 정하는 대안 질문 (블록 2) 을 내기 직전, 각 안이 기대는 전제를 이 저장소의 실제 코드 · 파일 · 기록 · 설정으로 확인한다. 그 안이 여기서 실제로 먹히는지를 보는 것이다.

| 확인할 것 | 예 |
|---|---|
| 데이터가 실제로 있는가 | 판정 기준으로 쓸 기록이 대상 중 몇 건에 있는가 (예: 워크트리 8개 중 그 기록이 있는 것은 2개) |
| 기존 코드가 전제를 지키는가 | 그 안이 기대는 규약을 지금 코드가 실제로 따르는가 |

확인 결과는 대안 비교에 그대로 싣는다 (예: "안 A 는 대상 8개 중 2개에만 먹힘"). 전제가 깨진 안은 추천하지 않고, 깨졌다는 사실을 비교표에 적는다. 첫 질문 (피처 이름) 전에는 탐색하지 않는다 — 첫 질문이 늦어진다. 탐색은 그 대안에 필요한 범위만 한다.

확인 결과는 대안을 고르는 데만 쓴다. **사용자가 말한 필요와 동기는 바꾸지 않는다** — 예를 들어 사용자가 "목록이 길어 찾기 힘들다" 고 했는데 이 PC 에 항목이 4개뿐이어도, 요구사항서 배경에 "지금은 불편이 없다" 고 적지 않는다. 사용자의 환경은 확인한 저장소와 다를 수 있다.
...
기술설계 묶음에서 사용자에게 묻는 것은 **사용자가 판단할 수 있는 것**뿐이다 — 사용자가 보게 될 동작 차이, 비용·속도·범위 같은 맞바꿈. 필터를 어느 파일에서 거를지, 내부 함수 구성, 데이터 형식 같은 순수 구현 결정은 묻지 않는다. 메인이 추천안으로 정하고 기술설계서의 결정 표에 "메인 추천" 으로 적는다. 이 결정들은 10 단계 승인 메시지의 기술설계서 요약에 한 줄씩 보여준다.

메인이 추천으로 정한 것이라도 **사용자가 화면이나 결과로 보게 되는 동작** (건너뛰는 대상, 안내 문구, 빈 결과 처리, 경계 입력 처리) 이면 순수 구현 결정이 아니다. 6 단계에서 요구사항서에도 요구 항목으로 올린다.
...
방향을 정하는 전제는 4 단계에서 대안을 내기 전에 이미 확인했다. 이 단계는 대화 중 확인하지 못한 나머지를 문서 쓰기 전에 마저 보는 단계다. 문서에 적을 사실 (개수 · 파일 · 호출처 · 경계 조건) 중 아직 안 본 것을 영향 범위의 코드에서 확인한다. 틀린 사실은 두 문서에 함께 들어가고 검증에서야 걸린다. 4 단계에서 확인한 것은 다시 보지 않는다.
...
| 저장소 실데이터 | 지금 저장소의 실제 파일 · 기록 · 설정이 결정의 전제를 깨는가 (4 단계에서 못 본 결정만) |
...
- 누가 정했든 (사용자 / 메인 추천) **사용자가 화면이나 결과로 보게 되는 동작**은 모두 요구 항목으로 올린다 — 건너뛰는 대상, 안내 문구, 빈 결과 처리, 경계 입력 처리. 메인이 추천으로 정한 것도 여기서 빠뜨리지 않는다
- 순수 구현 방식 (파일 위치, 함수 구성, 데이터 형식) 만 기술설계서 전용이다. 요구사항서에 옮기지 않는다
- 요구 항목에는 사용자가 보는 **결과**만 적는다 ("하이픈으로 시작하는 검색어도 찾아진다"). 그 결과를 내는 **처리 방법** (인코딩 정규화, 셸 인용, 인자 파싱 방식) 은 기술설계서에 적는다
...
- 4 · 5.5 에서 확인한 코드 사실만 적는다. 확인하지 않은 개수나 경로를 추측으로 채우지 않는다
- 쓰다가 요구사항서에 없는 사용자 동작 (위 6 단계 기준) 을 발견하면 기술설계서에만 적지 않는다. 요구사항서로 돌아가 요구 항목을 끝 번호 다음으로 더하고, 기술설계서는 그 번호로 가리킨다
...
두 문서 전문과 검증 보고서 전문은 싣지 않는다. 사용자가 판단에 필요한 핵심만 한 메시지에 모은다. 문서별로 끊어 승인받지 않는다. 목표 길이는 80줄 안팎이다.

| 순서 | 싣는 것 |
|---|---|
| 1 | 두 파일의 절대 경로 |
| 2 | 요구사항서 — 요구 항목 전체 목록 (짧으므로 그대로) |
| 3 | 기술설계서 — 결정 표에서 "메인 추천" 으로 정한 결정만 한 줄씩 (결정 / 고른 안 / 이유), 위험 제목 목록 |
| 4 | 검증 요약 — 검증자별 지적 수, 채택 · 기각 수, 9 단계에서 이미 고친 결함 한 줄씩, 남은 결함, 판단 사안 권장 |
| 5 | 규칙 원본 제목을 대신 쓴 경우 그 한 줄 (있을 때만) |

사용자가 결정한 항목은 대화에서 이미 본 것이라 다시 싣지 않는다. 메인 추천 결정 목록은 3 번 한 곳에만 싣는다 — 따로 목록을 만들지 않는다.

```json
{
  "question": "<slug>-requirements.md + <slug>-tech-design.md (+ 검증 보고서) 승인하고 진행? 두 문서 전문은 위 경로에서 열어 볼 수 있습니다.",
  "header": "두 문서 승인",
  "multiSelect": false,
  "options": [
...
수정 요청은 메인이 직접 고친다. 한 문서의 결정을 고치면 다른 문서의 같은 결정도 함께 고친다. 어느 문서든 바뀌었으면 8 · 9 를 다시 돌린 뒤 같은 요약 형식으로 다시 한 번에 보여준다. "어디를 고칠까요" 라고 되묻지 않는다.
...
| 메인 추천으로 정한 사용자 동작 (건너뛰는 대상 · 안내 문구 · 빈 결과 · 경계 입력) 을 기술설계서에만 적음 | 누가 정했든 사용자가 보는 동작이면 요구사항서에도 요구 항목으로 |
| 실데이터 확인 결과로 요구사항서의 배경 · 동기를 바꿔 적음 ("지금은 불편 없음") | 실데이터는 대안 선택에만. 사용자가 말한 필요는 그대로 |
| 요구 항목에 처리 방법 (정규화 · 셸 인용 · 파싱) 을 적음 | 요구 항목은 사용자가 보는 결과만. 처리 방법은 기술설계서 |
| 대안을 낸 뒤에야 저장소 실데이터를 확인해 되짚기 질문 | 대안 질문 전에 그 전제를 실제 코드 · 데이터로 확인하고 비교에 싣는다 |
| 첫 질문 (피처 이름) 전에 탐색 | 첫 질문은 바로. 탐색은 대안을 내기 직전에 |
| 문서별로 따로 승인 | 두 문서 + 보고서를 한 번에 |
| 승인 메시지에 두 문서 전문 · 보고서 전문을 실음 | 경로 + 핵심 요약 (80줄 안팎). 전문은 경로에서 연다 |
| 메인 추천 결정을 기술설계서 요약과 별도 목록으로 한 번 더 실음 | 기술설계서 요약 한 곳에만 |
...
| 코드를 다시 안 보고 문서 작성 | 4 · 5.5 에서 확인한 사실만 적는다 |
