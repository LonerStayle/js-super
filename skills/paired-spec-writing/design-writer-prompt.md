...
**치환 규칙:** `<TECH_DESIGN_SKILL_PATH>` / `<TECH_DESIGN_PATH>` / `<FEATURE_NAME>` / `<SLUG>` / `<REPO_ROOT>` / `<SUMMARY>` 여섯 자리만 채운다. `<REPO_ROOT>` 는 저장소 최상위 절대 경로다. `<SUMMARY>` 는 요구사항 작성 에이전트에게 넣는 것과 **한 글자도 다르지 않은** 텍스트여야 한다.
...
Agent tool (general-purpose):
  ...
  prompt: |
    ...
    - 코드베이스는 읽어도 된다 (읽기 전용): <REPO_ROOT>
      요약이 적은 코드 사실 (파일 존재 · 개수 · 호출처 · 함수 반환) 은 쓰기 전에
      직접 확인한다. 요약과 코드가 다르면 코드 기준으로 쓰고 보고의 Mismatch 에
      적는다
    ...
    - §5 에는 요약의 두 결정 표 (사용자가 보는 동작 결정 · 구현 방식 결정) 를
    ...
    Mismatch: <요약과 코드가 달랐던 사실과 코드 기준 값, 없으면 없음>
