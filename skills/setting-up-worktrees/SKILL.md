...
2. **설명만 있고 이름 없음** — 메인이 이름을 생성해 제안한다. **부모브랜치__자식이름 규칙**. 갈래는 **Step 0 출력의 `REBRANCH=` 줄만** 보고 고른다 (경로 · 기억 · 추측 금지, 출력이 안 보이면 Step 0 재실행):
   ...
   - **사용자가 베이스를 명시한 경우** (예: "dev 기준으로") — 이때만 `REBRANCH=` 줄 대신 그 베이스를 부모로 본다. Step 0 의 `REBRANCH=` 는 호출 위치 HEAD 로만 계산되기 때문이다. 베이스가 `MAIN_BRANCH` 와 다르면 `<베이스>__<자식이름>`, 같으면 접두어 없음. Step 4 의 부모 기록과 Step 6 의 보고도 같은 베이스를 쓴다 (Step 4 문단 참조).
