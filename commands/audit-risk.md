...
checked / title / evidence / impact / recommendation / summary / why_unverified /
how_to_check 등 사람이 읽는 값은 모두 한국어 문장으로 쓴다. 필드 이름과 status,
severity, category 값은 형식이므로 그대로 둔다.

영어를 그대로 두는 것: 파일 경로, 함수·변수·클래스 이름, 명령어, 라이브러리 이름,
설정 키 이름. 즉 그대로 검색해야 찾을 수 있는 것.

한국어로 푸는 것: 그 외 설명. 예를 들어 이렇게 바꾼다.

| 이렇게 쓰지 말고 | 이렇게 쓴다 |
|---|---|
| HTTP client 사용처 없음 | 외부와 통신하는 코드 없음 |
| SDK import 전수 확인 | 외부 라이브러리를 가져오는 부분 전부 확인 |
| grep 으로 검출 | 검색해서 확인 |
| loopback 바인딩 | 자기 컴퓨터에서만 접속을 받는 설정 |
| 실행 경로 unverified | 실제로 실행되는지 확인하지 못함 |
| manifest 파일 | 플러그인 설정 파일 |
| 하드코딩된 credential | 코드에 그대로 적힌 비밀값 |

영어 약어를 꼭 써야 하면 처음 나올 때 한국어 설명을 함께 적는다.
예: "CORS (다른 주소에서 오는 요청을 허용하는 설정)".
...
보고서 본문은 한국어 문장으로 씁니다. 파일 경로, 함수·변수 이름, 명령어, 라이브러리 이름처럼 그대로 검색해야 찾을 수 있는 것만 영어로 두고, 나머지 설명은 한국어로 풉니다. 영어 약어를 꼭 써야 하면 처음 나올 때 한국어 설명을 함께 적습니다.

보조 에이전트가 영어 섞인 문장을 돌려줬더라도, 보고서에 옮길 때 메인이 한국어로 다듬습니다. 사실 관계는 바꾸지 않고 표현만 바꿉니다.
