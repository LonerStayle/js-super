---
description: upstream superpowers 원본 writing-plans 흐름. /og-brainstorm 으로 만든 design doc 을 기반으로 implementation plan 을 작성. /og-write-plan 명시 호출 전용.
...
**시작 안내:** "og 원본 writing-plans 흐름으로 구현계획을 만들겠습니다."

**Context:** This should be run in a dedicated worktree (created by the `/og-brainstorm` flow).
...
- [ ] Execution Handoff — /og-execute-plan 안내
...
**2. Inline Execution** - Execute tasks in this session via `/og-execute-plan`, batch execution with checkpoints
...
`/og-execute-plan` — Inline 실행. 또는 Subagent-Driven 선택 시 untouched-upstream `subagent-driven-development` 사용.
