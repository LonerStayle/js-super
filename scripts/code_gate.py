...
class GateContext:
    ...
    python_tests_status: str | None = None   # C1 의 파이썬 결과 — C7 의 파이썬 경로가 먼저 본다
    mutation_deadline: float | None = None   # C7 전체의 마감 시각 — 두 언어가 예산을 나눠 쓴다
...
def _mutation_tool_name(mutation: dict, key: str, default: str, notes: list) -> str:
    """mutation 블록의 도구 이름 하나. 문자열이 아니면 기본값으로 되돌리고 사유를 남긴다."""
    value = mutation.get(key, default)
    if not isinstance(value, str) or not value.strip():
        notes.append(f"설정 파일의 mutation.{key} 값이 문자열이 아니라 기본값 {default} 을 사용했습니다.")
        return default
    return value.strip()
...
def _python_candidates(repo_root: Path) -> list:
    """인터프리터 후보를 우선순위대로. 활성화된 가상환경이 가장 앞이다."""
    candidates: list = []
...
def resolve_python(repo_root: Path) -> str:
    ...
    for cand in _python_candidates(repo_root):
        try:
            if cand.is_file():
                # 절대 경로로 돌려준다. C7 의 파이썬 경로는 이 인터프리터를 저장소 밖(캐시
                # 작업 디렉토리)에서 부르므로, `--repo-root .` 처럼 상대 경로로 들어오면
                # 그 자리에서 실행 파일을 찾지 못해 그 항목만 오류로 끝난다 (실측).
                # 심링크는 풀지 않는다. 가상환경의 `bin/python` 은 대개 시스템 인터프리터를
                # 가리키는 심링크라, 풀면 가상환경 밖으로 나가 그 환경의 패키지가 사라진다.
...
def probe_tools(repo_root: Path, python_exe: str, timeout: int = 30) -> dict:
    ...
    """
    tools: dict = {}
    # mutmut 도 파이썬 모듈이라 같은 서브프로세스 한 번에 함께 본다 — C7 의 파이썬 경로가
    # 쓴다. 도구 하나 확인하겠다고 프로세스를 더 띄우면 그만큼 R1 이 깎인다.
    modules = ("pytest", "coverage", "lizard", "mutmut")
...
def _python_tests_part(ctx: GateContext) -> dict:
    """C1 의 파이썬 조각. 예산을 넘겨 죽은 회차에도 상태를 남긴다.

    C7 의 파이썬 경로가 이 값을 먼저 본다. mutmut 은 기준 테스트가 통과해야만 돌아가는데,
    실패를 모르고 부르면 스위트를 두 번 더 돌리고 나서야 준비 실패로 끝난다 (R1).
    예외가 그대로 지나가면 상태가 비어 있고, C7 의 가드는 그것을 "아직 재지 않았다" 와
    구별하지 못해 통과된다 — 스위트 예산(초)을 못 지킨 저장소에서 그 스위트를 뮤테이션
    예산(기본 600초)으로 다시 돌리게 된다. 그래서 상태를 남기고 예외는 그대로 올려보낸다.
    """
    try:
        outcome = _run_python_tests(ctx)
    except subprocess.TimeoutExpired:
        ctx.python_tests_status = "timeout"
        raise
    ctx.python_tests_status = outcome.get("status")
    return outcome
...
def check_tests(ctx: GateContext) -> dict:
    parts = []
    if ctx.langs["python"]:
        parts.append(_python_tests_part(ctx))
...
def _mutation_budget(ctx: GateContext) -> int:
...
def _mutation_partial(ctx: GateContext, paths: dict, targets, elapsed: float, budget: int) -> dict:
    ...
    tail = _mutation_partial_gaps(summary, targets)
    return {
        "status": "timeout",
...
def _mutation_preconditions(ctx: GateContext, js_files, py_files) -> dict | None:
...
def _stryker_preconditions(ctx: GateContext) -> dict | None:
...
def _mutation_run(ctx: GateContext, paths: dict, cmd: list, targets, scope_notes: list) -> tuple:
...
def _check_mutation_javascript(ctx: GateContext, js_files) -> dict:
...
def _run_mutation_javascript(ctx: GateContext, js_files) -> tuple:
...
def build_mutmut_records(rel: str, meta: dict, spans: dict, mutant_source: str,
    ...
    그리고 함수 단위 테스트 목록이다.
    """
    mutant_lines = _split_source(mutant_source)
...
def _mutmut_test_roots(repo_root: Path, roots) -> tuple:
    ...
    found: list = []
    rejected: list = []
    for rel in detect_pytest_paths(repo_root):
...
def _mutmut_incremental_guard(ctx: GateContext, work: Path, targets, changed_tests) -> None:
    ...
    """
    mutants = Path(work) / "mutants"
    reason = _mutmut_stale_reason(_read_json_object(Path(work) / _MUTMUT_STATE_NAME),
...
def _mutmut_drop_mutants(ctx: GateContext, mutants: Path, reason: str) -> bool:
...
def _mutmut_write_state(ctx: GateContext, work: Path, targets) -> None:
...
def _mutmut_command(ctx: GateContext, work: Path, targets) -> tuple:
    """(실행할 명령, 결과가 나온 뒤에 남길 안내).
...
def _mutmut_partial(ctx: GateContext, summary: dict, targets, elapsed: float, budget: int) -> dict:
    """예산을 넘겨 중단했을 때 — 본 만큼만 내고 못 본 것을 반드시 말한다 (D4·R4).
...
def _mutmut_gap_notes(ctx: GateContext, summary: dict) -> None:
...
def _mutmut_decorator_gap(ctx: GateContext, targets) -> str:
    ...
    return (f"함수 {len(skipped)}개는 데코레이터가 붙어 있어 변이가 하나도 만들어지지 않았습니다 "
            f"(mutmut 이 건너뜁니다). 이 점수에 들어 있지 않습니다: {_sample_list(skipped)}")
...
def _mutmut_preconditions(ctx: GateContext) -> dict | None:
    ...
    tool = _tool(ctx, "mutmut")
    if not tool["available"]:
        return _skip("mutmut 이 설치돼 있지 않습니다.", "mutmut missing", tool["install_hint"])
    ...
    if status is not None and status != "ok":
        ...
        return _skip(_MUTMUT_TESTS_BLOCKED.get(status, _MUTMUT_TESTS_FAILED),
                     f"python tests not ok: {status}")
    return None


def _mutmut_scope(ctx: GateContext, py_files) -> tuple:
    """변이시킬 파일 목록과, 남는 것이 없을 때의 건너뜀 결과. 뺀 파일은 사유와 함께 남긴다."""
    targets, dropped = _mutmut_targets(ctx.repo_root, py_files)
    if dropped:
        ctx.notes.append(
            f"파이썬 뮤테이션 대상에서 테스트·삭제된 파일 {len(dropped)}개를 뺐습니다: "
            + _sample_list(list(dropped))
        )
    if not targets:
        return (), _skip(
...
def _mutmut_workspace(ctx: GateContext, roots) -> tuple:
...
def _mutmut_config_notes(ctx: GateContext, extra, carried: dict) -> None:
    ...
    if carried:
        ctx.notes.append(
            ...
            + _sample_list(sorted(carried))
        )


def _mutmut_setup(ctx: GateContext, targets, changed_tests) -> tuple:
    ...
    if unusable:
        ctx.notes.append(
            ...
            "못했습니다: " + _sample_list(list(unusable))
        )
    if not roots:
        return None, (), _skip("변이시킬 파이썬 소스 루트를 정하지 못했습니다.", "no python source root")
...
def _mutmut_uncovered(ctx: GateContext, work: Path, targets, elapsed: float) -> tuple:
    ...
    ctx.notes.append(
        ...
        "사라지지 않게 하기 위해서입니다.")
    return _mutation_outcome(ctx, summary, elapsed, targets), summary
...
def _mutmut_exit_note(ctx: GateContext, proc) -> None:
...
def _mutmut_finished(ctx: GateContext, work: Path, targets, proc, summary: dict,
                     elapsed: float) -> tuple:
...
def _mutmut_measured_notes(ctx: GateContext, summary: dict, scope_notes: list) -> None:
...
def _mutmut_timed_out(ctx: GateContext, summary: dict, targets, timing: tuple,
...
def _mutmut_run(ctx: GateContext, work: Path, cmd: list, targets, scope_notes: list) -> tuple:
    ...
    if budget <= 0:
        ...
        return _mutation_out_of_budget(
            "python", "파이썬", ctx.config.mutation_timeout_seconds)["outcome"], None
    started = time.perf_counter()
    proc = None
    timed_out = False
    try:
        proc = _run(cmd, cwd=work, timeout=budget)
    except subprocess.TimeoutExpired:
        timed_out = True
...
def _check_mutation_python(ctx: GateContext, py_files) -> dict:
...
def _run_mutation_python(ctx: GateContext, py_files) -> tuple:
...
def _mutation_changed_python_files(ctx: GateContext) -> list:
...
def _mutation_below(ctx: GateContext, parts) -> list:
...
def _mutation_merged_caveats(ctx: GateContext, parts) -> str:
    """합산 점수만 읽으면 놓치는 것 — 재지 못한 언어와 기준 미달 언어."""
    text = ""
    unmeasured = ", ".join(_mutation_unmeasured(parts))
    if unmeasured:
        text += f". {unmeasured}{_ko_topic(unmeasured)} 재지 못해 이 점수에 들어 있지 않습니다"
    below = ", ".join(_mutation_below(ctx, parts))
    if below:
        text += f". {below}{_ko_topic(below)} 기준에 못 미칩니다"
    return text


def _mutation_merged_head(ctx: GateContext, parts, counts: dict, per_language) -> str:
    ...
    text = (f"{lead} {score:.1f}% (기준 {ctx.config.mutation_score_threshold:g}%), "
            f"변이 {sum(counts.values())}개 — {_mutation_distribution(counts)} "
...
def _merge_mutation_languages(ctx: GateContext, parts: list) -> dict:
...
def _mutation_out_of_budget(language: str, label: str, budget: int) -> dict:
    ...
    return {
        ...
        "outcome": _skip(f"앞 언어가 뮤테이션 예산({budget}초)을 다 써 {label}{_ko_topic(label)} 재지 못했습니다. "
...
def check_mutation(ctx: GateContext) -> dict:
    ...
    py_files = _mutation_changed_python_files(ctx)
    ...
    parts = []
    if js_files:
        parts.append(_check_mutation_javascript(ctx, js_files))
    if py_files:
        parts.append(_check_mutation_python(ctx, py_files) if _mutation_budget(ctx) > 0
                     else _mutation_out_of_budget("python", "파이썬", budget))
...
def _resolve_repo_root(explicit) -> Path:
    """검사할 저장소 루트. 반드시 절대 경로다.

    C7 의 파이썬 경로는 저장소 밖(캐시 작업 디렉토리)에서 도구를 돌리고 소스를 그 안에
    심링크로 건다. 루트가 상대 경로면 그 링크가 작업 디렉토리 기준으로 풀려 아무 데도
    가리키지 않고, 사본에 소스도 테스트도 없이 "0 files mutated" 로 끝난다 (실측:
    `--repo-root .`). 심링크는 풀지 않는다 — 사용자가 준 경로 표기를 그대로 둔다.
    """
...
def _mutation_detail_line(finding: dict) -> str:
    ...
    변이 종류 이름은 도구에 따라 없을 수 있다 (파이썬 경로가 그렇다). 없으면 그렇다고
    적는다 — `None` 을 그대로 찍으면 게이트가 고장 난 것처럼 보인다.
    """
    status = MUTANT_STATUS_KO.get(finding.get("status"), finding.get("status") or "")
    kind = _shown_or(finding.get("mutator"), "변이 종류 없음")
