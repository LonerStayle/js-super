...
# ---------------------------------------------------------------------------
# C7 — 뮤테이션 (scripts/mutation/ 패키지)
#
# 언어 중립층(score) / 자바스크립트 어댑터(javascript) / 파이썬 어댑터(python) /
# 합치는 층(__init__)으로 나뉘어 있다. 뮤테이션 모듈은 뼈대를 모듈 객체 경유로만
# 참조한다 (`gate._run(...)`) — 테스트의 `monkeypatch.setattr(code_gate, "_run", ...)` 가
# 어댑터에도 먹히려면 호출 시점 조회여야 한다.
#
# 직접 실행(`python3 scripts/code_gate.py`)이면 이 파일이 `__main__` 으로 돌아
# `scripts.mutation` 을 찾지 못한다. 아래 세 줄이 그 경로를 살린다 — sys.path 에
# 저장소 루트를 넣고, 지금 실행 중인 모듈 자신을 `scripts.code_gate` 라는 이름으로
# 등록해 어댑터의 `from scripts import code_gate` 가 사본이 아닌 이 모듈을 받게 한다.
# 테스트 경로(`from scripts import code_gate`)에서는 __package__ 가 "scripts" 라
# 부트스트랩이 통째로 건너뛰어진다.
# ---------------------------------------------------------------------------
